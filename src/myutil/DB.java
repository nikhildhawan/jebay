...
public class DB
{
	...
	public void openConnection()
	{
		...
		try
		{
			...
			Connection con = DriverManager.getConnection(url, RuntimeSettings.dbUserID, RuntimeSettings.dbPassword);
	...
	public static int getCount(Connection con, String tableName, String whereClause)
	{
		int result = 0;
		String selectQuery = "SELECT count(*)  from " + tableName + " " + whereClause;
	...
	private DB(String password, String dbname)
	{
		...
		{
			Class.forName("com.mysql.jdbc.Driver");
			this.conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + dbname + "?autoReconnect=true", "root", password);
		}
	...
	/**
	 * executes the update SQL; if it is not successful, it will execute the insert SQL. If rows affected by the update query is 0, that means the update was not successful
	 * 
	 * @param updateSQL
	...
	public static int updateElseInsertTMS(String updateSQL, String insertSQL)
	{
		...
		if (!isOK)
		{
			MyLog.myIO("in updateElseInsertTMS, incorrect parameters:" + "updateSQL:" + updateSQL + ":, insertSQL:" + insertSQL + ":");
			System.exit(1);
		}
	...
	static void text2Table(String fileName)
	{
		...
		for (int i = 0; i < linesFromFile.length; i++)
		{
			if (linesFromFile[i].trim().startsWith("Running query :"))
			{
				...
				{
					con = getConnection();
					rs = readFromDB("show tables like 'temp_1_" + tableName + "'", con);
					if (!rs.next())
					{
						update("create table temp_1_" + tableName + " like " + tableName);
					}
			...
			if (linesFromFile[i].startsWith("row"))
			{
				...
				if (tableName.contains("sms_rcvd_tbl"))
				{// && fields.length >
					// 22) {

					update("insert into temp_1_" + tableName + " values(" + fieldValues.substring(0, (fieldValues.length() - 5)) + ");");
				}
				else
				{
					update("insert into temp_1_" + tableName + " values(" + fieldValues.substring(0, (fieldValues.length() - 1)) + ");");
				}
	...
	static String getFieldValues(String fields[])
	{
		String fieldValues = "";
		/*
		 * if (fields[0].contains(":")) { fields[0] = fields[0].substring(fields[0].indexOf(":") + 1, fields[0].length()); }
		 */
		for (int i = 0; i < fields.length; i++)
		{
			// System.out.println("i value:" + fields[i] + ":" + i);
			if ((fields[i].matches("[ ](20)[0-9][0-9][/](0[1-9]|1[012])[/]" + "(0[1-9]|[12][0-9]|3[01])")))
			{ // *** change to regexp
				// dd/dd/dddd
				// if (fields[i].contains("/")}{

				fieldValues += "'" + fields[i].trim() + ", " + fields[i + 1].trim() + "',";
				i++;
	...
	{
		Connection connection = null;
		System.out.println("in readFromDB with query:\n" + query);
		connection = getConnection();
	...
	public static ResultSet readFromDB(String query, Connection connection)
	{
		...
		try
		{
			...
			if ((query.indexOf(" where ") >= 0) || (query.indexOf("count ") >= 0))
			{
				// ok
			}
			else
	...
	public static Connection getConnection()
	{
		...
		try
		{
			...
			String connectionUrl = "jdbc:mysql://" + RuntimeSettings.SERVER_IP + ":" + RuntimeSettings.portNo + "/" + RuntimeSettings.databaseName + "?zeroDateTimeBehavior=convertToNull";
			con = DriverManager.getConnection(connectionUrl, RuntimeSettings.dbUserID, RuntimeSettings.dbPassword);
		}
		catch (SQLException ex)
		{
		...
		activeConnections++;
		MyLog.log("in DBDataStandAlone added new tms conn; total connections:" + activeConnections);
		return con;
	}
	...
	public static boolean listOracleDBTableNames(Connection conn)
	{
		...
		try
		{
			...
			for (int i = 0; i < tableNames.size(); i++)
			{
				...
				String query = "select * from " + tableName + " where ROWNUM < 4";
	...
	public static boolean listCountOfRowsInAllTables(String dbName, Connection myConn, Connection myConn2)
	{
		MyLog.myReport("\r\n\r\n\t\tSnapshot of Rows in tables of Database " + dbName + "\r\n\r\n");
		MyLog.log("\r\n\r\n\t\tSnapshot of Rows in tables of Database " + dbName + "\r\n\r\n");
		System.out.println("\r\n\r\n\t\tSnapshot of Rows in tables of " + "Database " + dbName + "\r\n\r\n");
		...
		try
		{
			...
			while (rs.next())
			{
				...
				try
				{
					...
					// continue;
					// }
					if ((tablename.contains("pkey")) || tablename.contains("pg_"))
					{
						continue;
					}
					rs2 = readFromDB("select count(*) from " + tablename + "", myConn2);
					if (rs2.next())
					{
						rows = rs2.getInt(1);
	...
	public static boolean countAllRows(String dbName)
	{
		...
		int i;
		int rows = 0;
		String[] tablename = { "sms_send_tbl", "sms_rcvd_tbl", "other_msg_tbl", "vehicle_assignment", "vid_mdt" };
		MyLog.myReport("\r\n\r\n\t\tSnapshot of Rows in tables of Database " + dbName + "\r\n\r\n");
		MyLog.log("\r\n\r\n\t\tSnapshot of Rows in tables of Database " + dbName + "\r\n\r\n");
		System.out.println("\r\n\r\n\t\tSnapshot of Rows in tables of Database " + dbName + "\r\n\r\n");
		MyLog.myReport("Table Name             \t\t # of Rows");
		...
		try
		{
			...
			{
				rs2 = readFromDB("select count(*) from " + tablename[i] + "", myConn2);
				if (rs2.next())
				{
					rows = rs2.getInt(1);
	...
	boolean runScriptFromFile(String dbName, String fileName)
	{
		...
		System.out.println("in runScriptFromFile with " + dbName + ", " + fileName);
		...
		try
		{
			...
			while ((rs.next()) && (i < 100))
			{
				...
				for (int j = 1; j <= columnCount; j++)
				{
					...
					if ((":DECIMAL:NUMERIC:INTEGER:".indexOf(":" + columnTypeName.toUpperCase() + ":") >= 0) || (columnTypeName.toUpperCase().indexOf("INT") >= 0) || (columnTypeName.toUpperCase().indexOf("BOOL") >= 0))
					{
	...
	public static void displayResultSet(ResultSet resultSet)
	{
		...
		try
		{
			...
			for (int j = 1; j <= columnCount; j++)
			{
				...
				MyLog.log("col name:" + columnName + ": columnValue:" + columnValue + ":");
			}
		}
		catch (SQLException ex)
		{
	...
	public String getTableColumnSpecifiers(String tableName)
	{
		...
		try
		{
			...
			for (int j = 1; j <= columnCount; j++)
			{
				...
				{
					result += "id INT(11) PRIMARY KEY AUTO_INCREMENT " + " NOT NULL,\r\n";
					continue;
				}
				String columnTypeName = rsmd.getColumnTypeName(j);
				...
				if (":DATETIME:TINYINT:DOUBLE:INT(11):".indexOf(":" + columnTypeName.toUpperCase() + ":") < 0)
				{
					result += "(" + precision;
	...
	public String getsetClassValuesFromResultSet(ResultSetMetaData rsmd)
	{
		...
		try
		{
			for (int i = 1; i <= rsmd.getColumnCount(); i++)
			{
				...
				if (rsmd.getColumnName(i).equalsIgnoreCase("id") || rsmd.getColumnName(i).equalsIgnoreCase("created_at") || rsmd.getColumnName(i).equalsIgnoreCase("updated_at"))
				{
					continue;
				}
				if (getJavaType(rsmd.getColumnTypeName(i)).equalsIgnoreCase("int"))
				{
					text += "            this.set" + Character.toUpperCase(rsmd.getColumnName(i).charAt(0)) + rsmd.getColumnName(i).substring(1, rsmd.getColumnName(i).length()) + "(rs.getInt(\"" + rsmd.getColumnName(i) + "\"));" + "\r\n";
				}
				else
				{
					text += "            this.set" + Character.toUpperCase(rsmd.getColumnName(i).charAt(0)) + rsmd.getColumnName(i).substring(1, rsmd.getColumnName(i).length()) + "(rs.getString(\"" + rsmd.getColumnName(i) + "\"));" + "\r\n";
				}
			}
	...
	public String getUpdateSql(String tablename)
	{
		...
		text += "        return updateTMS(\"update " + tablename + " \" + sql);" + "\r\n" + "    }";
		return text;
	...
	public String getToString(ResultSetMetaData rsmd)
	{
		...
		try
		{
			...
			for (int i = 1; i <= rsmd.getColumnCount() - 2; i++)
			{
				...
				{
					text += "\"" + rsmd.getColumnName(i) + " = \" + " + rsmd.getColumnName(i) + " + ";
				}
				else
				{
					text += "\"" + rsmd.getColumnName(i) + " = \" + " + rsmd.getColumnName(i);
	...
	/*
	 * public static Test find_via_id(int id) { Test test = new Test(); test = (Test)find_model_via_id(id,test); return test; }
	 */
	public String getInsertFind_via_id(String tablename, String modelName)
	{
		...
		text += "        " + modelName + " " + modelName.toLowerCase() + " = new " + modelName + "();\r\n";
		text += "        " + modelName.toLowerCase() + " = (" + modelName + ")find_model_via_id(id, " + modelName.toLowerCase() + ");\r\n";
	...
	public String getInsertUpdate(ResultSetMetaData rsmd, String tablename)
	{
		...
		try
		{
			...
			text += ";" + "\r\n" + "        int rows = updateTMS(\"update " + tablename + " set \" + fields + \"where id = \"+" + rsmd.getColumnName(1) + ");";
	...
	public String getGetInsertSql(ResultSetMetaData rsmd, String tablename)
	{
		...
		try
		{
			for (int i = 1; i <= rsmd.getColumnCount(); i++)
			{
				if (rsmd.getColumnName(i).equalsIgnoreCase("created_at") || rsmd.getColumnName(i).equalsIgnoreCase("updated_at"))
				{
					continue;
				}
			...
			for (int i = 1; i <= rsmd.getColumnCount(); i++)
			{
				...
				if (getJavaType(rsmd.getColumnTypeName(i)).equalsIgnoreCase("" + "String"))
				{
					if (i == 2)
					{
						text += "\", '\" + " + rsmd.getColumnName(i) + " + \"'";
	...
	public String getSetterGetters(ResultSetMetaData rsmd)
	{
		...
		try
		{
			for (int i = 1; i <= rsmd.getColumnCount(); i++)
			{
				...
				text += "    public " + getJavaType(rsmd.getColumnTypeName(i)) + " get" + Character.toUpperCase(rsmd.getColumnName(i).charAt(0)) + rsmd.getColumnName(i).substring(1, rsmd.getColumnName(i).length()) + "() " + "{" + "\r\n" + "        return " + rsmd.getColumnName(i) + ";" + "\r\n" + "    }" + "\r\n";
				...
				text += "    public void set" + Character.toUpperCase(rsmd.getColumnName(i).charAt(0)) + rsmd.getColumnName(i).substring(1, rsmd.getColumnName(i).length()) + "(" + getJavaType(rsmd.getColumnTypeName(i)) + " " + rsmd.getColumnName(i) + ") {" + "\r\n" + "        this." + rsmd.getColumnName(i) + " = " + rsmd.getColumnName(i) + ";" + "\r\n" + "    }" + "\r\n";
			}
		}
		catch (SQLException ex)
		{
	...
	{
		String dataType = "";
		if (rsmdColumnType.equalsIgnoreCase("varchar") || rsmdColumnType.equalsIgnoreCase("char") || rsmdColumnType.equalsIgnoreCase("TIMESTAMP"))
		{
			dataType = "String";
		}
		else if (rsmdColumnType.equalsIgnoreCase("INT") || rsmdColumnType.equalsIgnoreCase("INT UNSIGNED") || rsmdColumnType.equalsIgnoreCase("DECIMAL") || (rsmdColumnType.toUpperCase().indexOf("INTEGER") >= 0))
		{
			dataType = "int";
		}
